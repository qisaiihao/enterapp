<template>
    ...
    <view class=\"ek-layer ek-fill\">
        ...
        <text class=\"ek-arrow\">{{ preloadCompleted ? '↵' : '⏳' }}</text>
            ...
            <view v-if="showEnterButton" :class="'enter-key-container ' + (preloadCompleted ? '' : 'loading')" @tap="handleEnterButtonClick">
                <view class="enter-key-l-shape">
                    <view class="enter-key-horizontal">
                        <text class="enter-key-label">{{ preloadCompleted ? 'Enter' : 'Loading' }}</text>
                        <text class="enter-key-arrow">{{ preloadCompleted ? '↵' : '⏳' }}</text>
                    </view>
                    <view class="enter-key-vertical"></view>
                </view>
            </view>
        </view>
...
.enter-key-horizontal, .enter-key-vertical, .enter-key-arrow, .enter-key-label { display: none !important; width: 0; height: 0; overflow: hidden; }

/* New L-key layers */
.ek-layer { position: absolute; inset: 0; }
.ek-border { background: #333; filter: drop-shadow(0 10rpx 16rpx rgba(0,0,0,.18));
  clip-path: polygon(58% 0,100% 0,100% 100%,0 100%,0 60%,58% 60%,58% 0); border-radius: 24rpx; }
.ek-fill { background: #fff; clip-path: polygon(60% 2%,100% 2%,100% 100%,2% 100%,2% 62%,60% 62%,60% 2%); border-radius: 22rpx; }
.ek-label { position: absolute; bottom: 26rpx; left: 22rpx; font-size: 22rpx; color: #333; font-weight: 500; }
.ek-arrow { position: absolute; bottom: 26rpx; left: 120rpx; font-size: 26rpx; color: #333; font-weight: 700; }
...
.ek-layer { position: absolute; inset: 0; }
.ek-border { background: #333; filter: drop-shadow(0 10rpx 16rpx rgba(0,0,0,.18)); clip-path: polygon(58% 0,100% 0,100% 100%,0 100%,0 60%,58% 60%,58% 0); border-radius: 24rpx; }
.ek-fill { background: #fff; clip-path: polygon(60% 2%,100% 2%,100% 100%,2% 100%,2% 62%,60% 62%,60% 2%); border-radius: 22rpx; }
.ek-label { position: absolute; bottom: 26rpx; left: 22rpx; font-size: 22rpx; color: #333; font-weight: 500; }
.ek-arrow { position: absolute; bottom: 26rpx; left: 120rpx; font-size: 26rpx; color: #333; font-weight: 700; }
</style>
