...
export default {
    ...
    methods: {
        ...
        callCloudFunction(name, data = {}) {
            console.log(`🔍 [Messages页面] 调用云函数: ${name}`, data);
            ...
            return new Promise((resolve, reject) => {
                ...
                console.log(`🔍 [Messages页面] 运行环境检测 - H5: ${isH5}, 小程序: ${isMiniProgram}`);
                
                if (isH5) {
                    // H5环境使用TCB
                    if (this.$tcb && this.$tcb.callFunction) {
                        console.log(`🔍 [Messages页面] H5环境使用TCB调用云函数: ${name}`);
                        this.$tcb.callFunction({
                            name: name,
                            data: data
                        }).then(resolve).catch(reject);
                    } else {
                        console.error(`❌ [Messages页面] H5环境TCB不可用`);
                        reject(new Error('TCB实例不可用'));
                    }
                } else if (isMiniProgram) {
                    // 小程序环境使用微信云开发
                    if (wx.cloud && wx.cloud.callFunction) {
                        console.log(`🔍 [Messages页面] 小程序环境使用微信云开发调用云函数: ${name}`);
                        wx.cloud.callFunction({
                            name: name,
                            data: data,
                            success: (res) => {
                                console.log(`✅ [Messages页面] 云函数调用成功: ${name}`, res);
                                resolve(res);
                            },
                            fail: (err) => {
                                console.error(`❌ [Messages页面] 云函数调用失败: ${name}`, err);
                                reject(err);
                            }
                        });
                    } else {
                        console.error(`❌ [Messages页面] 小程序环境微信云开发不可用`);
                        reject(new Error('微信云开发不可用'));
                    }
                } else {
                    console.error(`❌ [Messages页面] 未知运行环境`);
