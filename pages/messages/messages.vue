...
export default {
    ...
    methods: {
        ...
        callCloudFunction(name, data = {}) {
            console.log(`🔍 [消息页] 调用云函数: ${name}`, data);
            ...
            return new Promise((resolve, reject) => {
                ...
                console.log(`🔍 [消息页] 运行环境检测 - 平台: ${platform}, 方法: ${method}`);
                ...
                if (method === 'tcb') {
                    ...
                    if (this.$tcb && this.$tcb.callFunction) {
                        ...
                        console.error(`❌ [消息页] TCB实例不可用`);
                        reject(new Error('TCB实例不可用'));
                    }
                } else if (method === 'wx-cloud') {
                    // 使用微信云开发调用云函数（小程序环境）
                    if (wx.cloud && wx.cloud.callFunction) {
                        console.log(`🔍 [消息页] 小程序环境调用云函数: ${name}`);
                        wx.cloud.callFunction({
                            name: name,
                            data: data,
                            success: (res) => {
                                console.log(`✅ [消息页] 云函数调用成功: ${name}`, res);
                                resolve(res);
                            },
                            fail: (err) => {
                                console.error(`❌ [消息页] 云函数调用失败: ${name}`, err);
                                reject(err);
                            }
                        });
                    } else {
                        console.error(`❌ [消息页] 微信云开发不可用`);
                        reject(new Error('微信云开发不可用'));
                    }
                } else {
                    console.error(`❌ [消息页] 不支持的云函数调用方式: ${method}`);
