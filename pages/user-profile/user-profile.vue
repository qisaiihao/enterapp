...
export default {
    ...
    methods: {
        ...
        callCloudFunction(name, data = {}) {
            console.log(`🔍 [UserProfile页面] 调用云函数: ${name}`, data);
            ...
            return new Promise((resolve, reject) => {
                ...
                console.log(`🔍 [UserProfile页面] 运行环境: ${platform}, 调用方式: ${method}, 实际能力: ${capability}`);
                ...
                console.log(`🔍 [UserProfile页面] 最终使用调用方式: ${actualMethod}`);
                
                if (actualMethod === 'tcb') {
                    // 使用TCB调用云函数（H5和App环境）
                    if (this.$tcb && this.$tcb.callFunction) {
                        console.log(`🔍 [UserProfile页面] 使用TCB调用云函数: ${name} (环境: ${platform})`);
                        this.$tcb.callFunction({
                            name: name,
                            data: data
                        }).then(resolve).catch(reject);
                    } else {
                        console.error(`❌ [UserProfile页面] ${platform}环境TCB不可用`);
                        console.error(`❌ [UserProfile页面] this.$tcb:`, this.$tcb);
                        console.error(`❌ [UserProfile页面] this.$tcb.callFunction:`, typeof (this.$tcb && this.$tcb.callFunction));
                        reject(new Error('TCB实例不可用'));
                    }
                } else if (actualMethod === 'wx-cloud') {
                    // 使用微信云开发调用云函数（小程序环境）
                    if (wx.cloud && wx.cloud.callFunction) {
                        console.log(`🔍 [UserProfile页面] 使用微信云开发调用云函数: ${name}`);
                        wx.cloud.callFunction({
                            name: name,
                            data: data,
                            success: (res) => {
                                console.log(`✅ [UserProfile页面] 云函数调用成功: ${name}`, res);
                                resolve(res);
                            },
                            fail: (err) => {
                                console.error(`❌ [UserProfile页面] 云函数调用失败: ${name}`, err);
                                reject(err);
                            }
                        });
                    } else {
                        console.error(`❌ [UserProfile页面] 小程序环境微信云开发不可用`);
                        console.error(`❌ [UserProfile页面] wx.cloud:`, typeof wx.cloud);
                        console.error(`❌ [UserProfile页面] wx.cloud.callFunction:`, typeof (wx.cloud && wx.cloud.callFunction));
                        reject(new Error('微信云开发不可用'));
                    }
                } else {
                    console.error(`❌ [UserProfile页面] 不支持的云函数调用方式: ${actualMethod}`);
        ...
        loadUserPosts: function () {
            ...
            this.callCloudFunction('getUserProfile', {
                    ...
                    if (res.result && res.result.success) {
                        ...
                        const newUserPosts = this.userPosts.concat(posts);
                        this.setData({
        ...
        onImageLoad: function (e) {
            ...
            if (type === 'multi' && imgindex === 0) {
                ...
                query
                    .select(`#user-swiper-img-${postindex}-0`)
            ...
            if (type === 'single') {
                ...
                if (actualRatio < minRatio) {
                    const query = uni.createSelectorQuery().in(this);
                    query
                        .select(`#user-single-img-${postindex}`)
                        .boundingClientRect((rect) => {
                            if (rect && rect.width) {
                                const containerWidth = rect.width;
                                const displayHeight = containerWidth / minRatio;
                                if (this.imageClampHeights[postindex] !== displayHeight) {
                                    this.setData({
                                        [`imageClampHeights.${postindex}`]: displayHeight
                                    });
                                }
                            }
                        })
