...
exports.main = async (event, context) => {
  ...
  const { skip = 0, limit = 10, isPoem, isOriginal, isDiscussion, tag = '', author = '', includeActivity = false, activityId = '' } = event; // 添加author参数用于诗人筛选
  ...
  */
  ...
  try {
    ...
    if (isActivityQuery) {
      matchConditions.isActivityPost = true;
      matchConditions.activityId = safeActivityId;
    } else {
      // 常规列表默认排除活动帖子
