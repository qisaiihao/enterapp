        ...
        return new Promise((resolve, reject) => {
            ...
            if (isH5) {
                ...
                if (typeof getApp !== 'undefined' && getApp().$tcb && getApp().$tcb.callFunction) {
                    console.log(`🔍 [AvatarCache] H5环境使用TCB调用云函数: ${name}`);
                    getApp().$tcb.callFunction({
                        name: name,
                        data: data
                    }).then(resolve).catch(reject);
                } else {
                    console.error(`❌ [AvatarCache] H5环境TCB不可用`);
                    reject(new Error('TCB实例不可用'));
                }
            } else if (isMiniProgram) {
                // 小程序环境使用微信云开发
                if (wx.cloud && wx.cloud.callFunction) {
                    console.log(`🔍 [AvatarCache] 小程序环境使用微信云开发调用云函数: ${name}`);
                ...
                console.error(`❌ [AvatarCache] 未知运行环境`);
