...
exports.main = async (event, context) => {
  ...
  */
  ...
  try {
    ...
    // 如果指定了 tag 参数，添加标签筛选条件
    if (tag) {
      matchConditions.tags = tag;  // 匹配包含该标签的文档
      matchConditions['tags.0'] = { $exists: true };  // 确保 tags 数组至少有一个元素
      console.log('🔍 [getPostList] 添加tag筛选条件:', tag);
    }

    // 如果指定了 isDiscussion 参数，添加讨论筛选条件
    ...
    let posts = [];
    let timeOrderedPosts = []; // 在外层声明，便于在日志输出时使用
    let randomPosts = []; // 在外层声明，便于在日志输出时使用
    ...
    if (enableRandomMix) {
      ...
      try {
        ...
        // 复制所有筛选条件（虽然这里理论上没有筛选条件，但为了保险仍保留）
        // 如果有时间顺序的帖子，排除它们
        ...
        if (totalPosts > 0) {
          ...
          if (totalPosts <= targetCandidates) {
            ...
            for (let i = 0; i < queriesPerBatch && candidatePosts.length < targetCandidates; i++) {
              ...
              const randomQuery = db.collection('posts').aggregate()
                .match(randomPostsMatchConditions)
                .sort({ createTime: -1 })  // 虽然按时间排序，但 skip 是随机的
                .skip(randomSkip)
                .limit(perBatchCount);
          ...
          // 如果候选帖子还不够，从所有帖子中随机选择 skip 位置再获取
    ...
    // 双重保险：前端再次过滤被屏蔽用户的帖子（包括匿名帖子的 realAuthorOpenid）
    if (blockedUserIds.length > 0) {
    ...
    // --- 优化图片URL转换逻辑 ---
    const fileIDs = new Set(); // 使用 Set 避免重复 fileID
