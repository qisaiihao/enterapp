...
- 创作历程：按北京时间聚合整年原创发布记录，年度贡献图、年份切换、手机月历、点击日期回看。统计不依赖当前已加载的分页。
...
`posts`、`users`、`follows`、`blocks` 仅被读取。没有修改外层 uni-app 的源码、现有云函数、数据库权限、字体管理器或 App 资源。网站的汇文明朝是独立复制的字体文件；App 已验收的加载流程未改变。
...
- 14 项单元／接口模拟测试通过，包括越权拒绝、匿名信息隔离、隐藏作品、会话绑定、退出撤销、限流、日期和分页校验。
...
- **尚未用你的真实账号完成成功登录、个人资料和贡献图的端到端验收。** 没有提供测试账号；这些成功路径已在模拟接口和示例页面测试。
...
`preview` 地址为 `http://127.0.0.1:4173/`。当前使用 hash 路由，直达链接形式为 `/#/poems?id=...`，静态服务器无需路径重写。正式部署时，把目标域名加入腾讯云开发安全来源，使用 HTTPS，并将 `dist/` 部署到网站根路径。不要直接向现有 App 官网根目录覆盖发布。
